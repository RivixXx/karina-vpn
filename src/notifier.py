...
def quota_text(stage, used, quota):
    ...
    if stage == 100: text = "Лимит резервного мобильного подключения исчерпан.\n\nОсновные VPN-подключения продолжают работать."
    elif stage == 95: text = f"В резервном мобильном подключении осталось совсем немного трафика.\n\nИспользовано {used_gb:.1f} ГБ из {quota_gb:.0f} ГБ."
    else: text = f"Резервное мобильное подключение использовало 80% доступного трафика.\n\nОсталось примерно {left_gb:.1f} ГБ из {quota_gb:.0f} ГБ."
